...
class Luhn {
  static boolean test(String creditCard) {
    ...
    Integer s1 = oddEvens.get(false).stream().reduce(0, (x,y) -> x + y);
    Integer s2 = oddEvens.get(true).stream()
      .map(x -> {
        Integer doubled = x * 2;
        return (doubled > 9) ? (doubled % 10) + 1:  doubled;
      })
      .reduce(0, (x,y) -> x + y);
  ...
  public static void main(String[] args) {
    ...
    creditCards.stream().filter(Luhn::test).forEach(System.out::println);
  }
}